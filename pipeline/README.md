...
Extracts planning applications from the PlanIt API for areas 304 (Greenwich), 318 (Newham), 323 (Tower Hamlets) and processes them through a three-stage ETL pipeline: **Extract → Transform → Load** with in-memory DataFrames and AWS S3/DynamoDB integration.
...
The ETL pipeline can run on AWS Lambda via Docker. The `handler()` function in `load.py` executes the full pipeline (extract → transform → load with `--save-pdf`).

### Build & Deploy

Authenticate with ECR, build the image, and push to the repository:
...
docker build --platform linux/amd64 --provenance=false --sbom=false -t c25-planning-etl-repo:latest .

# Tag the image for ECR
docker tag c25-planning-etl-repo:latest $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/c25-planning-etl-repo:latest

# Push to ECR
docker push $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/c25-planning-etl-repo:latest
```
---
