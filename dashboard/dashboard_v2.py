...
def load_all_applications():
    ...
    try:
        ...
        required_cols = ["uid", "address", "app_type", "app_state", "location_x", "location_y", "start_date"]
        ...
        if "area_name" not in df.columns:
            ...
            df["area_name"] = df["uid"].apply(
                lambda x: str(x).split("_")[0] if pd.notna(x) and "_" in str(x) else "Unknown"
            )
        ...
        st.error("❌ Unable to load planning applications. Please check your AWS configuration.")
        return pd.DataFrame()
...
def load_heritage_sites():
    ...
    try:
        ...
        heritage_sites = get_sites(latitude=51.51, longitude=-0.01, radius=20000)
...
def load_conservation_areas_data():
    ...
    try:
        ...
        conservation_areas = get_conservation_areas(latitude=51.51, longitude=-0.01, radius=20000)
...
def filter_applications(df, selected_areas, selected_types, selected_statuses, date_range, search_query):
    ...
    # Filter by area
    if selected_areas:
        filtered_df = filtered_df[filtered_df["area_name"].isin(selected_areas)]
    ...
    if selected_statuses:
        filtered_df = filtered_df[filtered_df["app_state"].isin(selected_statuses)]
    ...
    if search_query:
        ...
        filtered_df = filtered_df[
            ...
            (filtered_df["uid"].astype(str).str.lower().str.contains(search_lower, na=False))
        ]
...
def setup_sidebar_filters(df):
    ...
    # Area filter
    available_areas = sorted(df["area_name"].unique().tolist()) if not df.empty else []
    ...
    available_types = sorted(df["app_type"].unique().tolist()) if not df.empty else []
    ...
    available_statuses = sorted(df["app_state"].unique().tolist()) if not df.empty else []
    ...
    show_heritage_sites = st.sidebar.checkbox("Heritage Sites", value=True, help="Show heritage sites from NHLE")
    show_conservation_areas = st.sidebar.checkbox("Conservation Areas", value=True, help="Show conservation area boundaries")
    show_clustering = st.sidebar.checkbox("Marker Clustering", value=True, help="Group markers at low zoom levels")
...
def build_folium_map(df, heritage_sites, conservation_areas, filters):
    ...
    if df.empty:
        st.warning("⚠️ No planning applications to display. Please adjust your filters.")
        return None
    ...
    try:
        ...
        logger.warning(f"Error calculating map center: {err}. Defaulting to Newham.")
        center_lat = newham_lat
        center_lon = newham_lon
    ...
    if filters["show_clustering"]:
        ...
        marker_cluster = MarkerCluster(name="Planning Applications (Clustered)").add_to(m)
        ...
        app_layer = folium.FeatureGroup(name="Planning Applications", show=True).add_to(m)
    ...
    if filters["show_heritage_sites"] and heritage_sites:
        heritage_layer = folium.FeatureGroup(name="Heritage Sites", show=True).add_to(m)
        
        for site in heritage_sites:
            try:
                if "geometry" in site and site["geometry"].get("type") == "Point":
                    coords = site["geometry"]["coordinates"]
                    props = site.get("properties", {})
                    
                    popup_text = f"""
                    <div style="font-family: Arial; font-size: 12px; width: 200px;">
                        <b>{props.get('Name', 'Heritage Site')}</b><br>
                        <b>Grade:</b> {props.get('Grade', 'N/A')}<br>
                        <a href="{props.get('Hyperlink', '#')}" target="_blank">View Details</a>
                    </div>
                    """
                    ...
                    folium.Marker(
                        ...
                        tooltip=props.get('Name', 'Heritage Site'),
                        icon=folium.Icon(color="blue", icon="info-sign", prefix="fa"),
                    ).add_to(heritage_layer)
            except Exception as err:
                logger.debug(f"Error processing heritage site: {err}")
                continue
    
    # ==================== CONSERVATION AREAS LAYER ====================
    
    if filters["show_conservation_areas"] and conservation_areas:
        conservation_layer = folium.FeatureGroup(name="Conservation Areas", show=True).add_to(m)
...
def main():
    ...
    if df_all.empty:
        st.error("❌ No data available. Please check your AWS configuration and try again.")
        return
    ...
    if df_filtered.empty:
        st.warning("⚠️ No applications match your filters. Try adjusting your selection.")
    else:
        with st.spinner("🗺️ Building map..."):
            m = build_folium_map(df_filtered, heritage_sites, conservation_areas, filters)
    ...
    if not df_filtered.empty:
        display_cols = ["uid", "address", "app_type", "app_state", "area_name", "start_date"]
        ...
        st.info(f"✅ Showing {len(df_filtered)} of {len(df_all)} total applications")
    else:
